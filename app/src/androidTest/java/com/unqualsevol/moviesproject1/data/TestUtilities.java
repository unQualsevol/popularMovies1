...
import com.unqualsevol.moviesproject1.utils.PollingCheck;
...
import static com.unqualsevol.moviesproject1.data.MoviesContract.MovieEntry.COLUMN_MOVIE_ID;
import static com.unqualsevol.moviesproject1.data.MoviesContract.MovieEntry.COLUMN_ORIGINAL_TITLE;
import static com.unqualsevol.moviesproject1.data.MoviesContract.MovieEntry.COLUMN_OVERVIEW;
import static com.unqualsevol.moviesproject1.data.MoviesContract.MovieEntry.COLUMN_POSTER;
import static com.unqualsevol.moviesproject1.data.MoviesContract.MovieEntry.COLUMN_POSTER_PATH;
import static com.unqualsevol.moviesproject1.data.MoviesContract.MovieEntry.COLUMN_RELEASE_DATE;
import static com.unqualsevol.moviesproject1.data.MoviesContract.MovieEntry.COLUMN_TITLE;
import static com.unqualsevol.moviesproject1.data.MoviesContract.MovieEntry.COLUMN_VOTE_AVERAGE;
...
class TestUtilities {
    ...
    static final int MOVIE_ID_TEST = 147528;
    ...
    static void validateCurrentRecord(String error, Cursor valueCursor, ContentValues expectedValues) {
        ...
        int posterColumn = valueCursor.getColumnIndex(MoviesContract.MovieEntry.COLUMN_POSTER);
        ...
        for (Map.Entry<String, Object> entry : valueSet) {
            ...
            String actualValue;
            if (index == posterColumn) {
                expectedValue = new String((byte[]) entry.getValue());
                actualValue = new String(valueCursor.getBlob(index));
            } else {
                expectedValue = entry.getValue().toString();
                actualValue = valueCursor.getString(index);
            }
    ...
    static ContentValues createTestMovieContentValues() {
        ...
        movieValues.put(COLUMN_MOVIE_ID, MOVIE_ID_TEST);
        movieValues.put(COLUMN_TITLE, "title");
        movieValues.put(COLUMN_ORIGINAL_TITLE, "original_title");
        movieValues.put(COLUMN_POSTER_PATH, "poster_path");
        movieValues.put(COLUMN_POSTER, "poster".getBytes());
        movieValues.put(COLUMN_OVERVIEW, "overview");
        movieValues.put(COLUMN_VOTE_AVERAGE, 5.5);
        movieValues.put(COLUMN_RELEASE_DATE, "release_date");
     ...
     */
    static ContentValues[] createBulkInsertTestMoviesValues() {

        ContentValues[] bulkTestWeatherValues = new ContentValues[BULK_INSERT_RECORDS_TO_INSERT];

        int testId = TestUtilities.MOVIE_ID_TEST;

        for (int i = 0; i < BULK_INSERT_RECORDS_TO_INSERT; i++) {

            ContentValues movieValues = new ContentValues();

            movieValues.put(COLUMN_MOVIE_ID, testId++);
            movieValues.put(COLUMN_TITLE, "title" + i);
            movieValues.put(COLUMN_ORIGINAL_TITLE, "original_title" + i);
            movieValues.put(COLUMN_POSTER_PATH, "poster_path" + i);
            movieValues.put(COLUMN_POSTER, ("poster" + i).getBytes());
            movieValues.put(COLUMN_OVERVIEW, "overview" + i);
            movieValues.put(COLUMN_VOTE_AVERAGE, 5.5 + 0.2 * i);
            movieValues.put(COLUMN_RELEASE_DATE, "release_date" + i);

            bulkTestWeatherValues[i] = movieValues;
        }
